...
const FailedTask = ({failedTaskData}) => {
  return (
    <>
        <div className="h-full flex-shrink-0 w-[300px] bg-gray-800 rounded-xl p-4 flex flex-col shadow-lg transition-transform transform hover:scale-105">
            ...
            <h1 className="text-white text-xl font-bold mb-1">{failedTaskData.title}</h1>
            <p className="text-gray-300 mb-4 overflow-hidden text-ellipsis whitespace-nowrap">{failedTaskData.description}</p>

            {/* Action Buttons */}
            <div className="flex gap-3 mt-auto">
                <button className="w-full bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition duration-200 ease-in-out focus:outline-none focus:ring focus:ring-red-300">Failed</button>
            </div>
        </div>
    </>
  )
